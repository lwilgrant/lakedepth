...
#testplot sofi data
# os.chdir(txtDIR)
# sofi_depths = pd.read_csv('dataframe_sofi.csv')
# sofi_depths = sofi_depths.set_index(keys=['Lat','Lon'])
# sofi_depths = sofi_depths.drop(columns='Unnamed: 0')
# ds_sofi = xr.Dataset.from_dataframe(sofi_depths)
# ds_sofi = ds_sofi.reindex_like(da_depth)
# da_sofi = ds_sofi['Mean']
# da_mean = ds_depths['GLDB_avg_mean_depth']
...
# da_mean.plot(ax=ax2,
#              transform=ccrs.PlateCarree())
# f.savefig(outDIR+'/sofi_luke_v2.png',bbox_inches='tight',dpi=400)
...
# cb_eof.ax.xaxis.label.set_size(title_font)
